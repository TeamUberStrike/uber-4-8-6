...
public class Water : MonoBehaviour
{
	...
	private void Update()
	{
		if ((bool)base.GetComponent<Renderer>())
		{
			...
			{
				// Standard-shader fallback: when the legacy FX/Water properties
				// aren't present (ForgeRipper remapped to Standard in Unity 6),
				// scroll _MainTex_ST.zw (UV offset) over time so the water at
				// least appears to flow instead of being a static texture.
				if (!sharedMaterial.HasProperty("_WaveScale") || !sharedMaterial.HasProperty("WaveSpeed"))
				{
					if (sharedMaterial.HasProperty("_MainTex_ST"))
					{
						Vector4 st = sharedMaterial.GetVector("_MainTex_ST");
						float t = Time.timeSinceLevelLoad;
						st.z = Mathf.Repeat(t * 0.02f, 1f);
						st.w = Mathf.Repeat(t * 0.01f, 1f);
						sharedMaterial.SetVector("_MainTex_ST", st);
					}
					return;
				}
