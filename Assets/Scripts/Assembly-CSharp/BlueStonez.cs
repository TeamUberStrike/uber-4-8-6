...
public static class BlueStonez
{
	...
	public static void Initialize(GUISkin skin)
	{
		...
		// Unity 6 IMGUI cannot render the legacy bitmap Font assets that came
		// from the Unity 4 export (InterparkGothicBold_*pt). Every GUI.Label
		// call against these styles spams "Unable to find a font file..."
		// thousands of times per second. Replace the font on every GUIStyle
		// in the skin with Unity's built-in legacy runtime font so IMGUI has
		// SOMETHING valid to render with. Fonts will look generic but no spam.
		...
		if (fallbackFont != null && Skin != null)
		{
			...
			{
				if (gs != null && gs.font == null) gs.font = fallbackFont;
				else if (gs != null && gs.font != null && gs.font.dynamic == false) gs.font = fallbackFont;
			}
			...
			// LocalizationHelper.GetLocalizedStyle returns fresh GUIStyle copies,
			// so the static fields on this class hold their own .font refs
			// outside Skin.customStyles. Walk them via reflection and patch too.
			FieldInfo[] fields = typeof(BlueStonez).GetFields(BindingFlags.Public | BindingFlags.Static);
