...
public class WeaponController : Singleton<WeaponController>, IWeaponController
{
	...
	{
		// Null guards: _weapons array and slot entry can be null during scene
		// transition or before loadout init completes — firing 89 NullRef
		// spam per session on input events otherwise.
		if (_weapons == null || _currentSlotID == null)
		{
			return;
		}
		WeaponSlot currentSlot = (_currentSlotID.Current >= 0 && _currentSlotID.Current < _weapons.Length) ? _weapons[_currentSlotID.Current] : null;
		if (!ev.IsDown || LevelCamera.IsZoomedIn || currentSlot == null || slotType == currentSlot.Slot || GetWeaponCount() <= 1)
		{
			return;
		}
		for (int i = 0; i < _weapons.Length; i++)
