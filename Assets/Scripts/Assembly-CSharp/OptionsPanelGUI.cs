...
public class OptionsPanelGUI : PanelGuiBase
{
	...
	private void DoVideoGroup()
	{
		...
		num5 += 30;
		GUI.Label(new Rect(8f, num5, 120f, 30f), "Field of View", BlueStonez.label_interparkbold_11pt_left);
		float num7 = GUI.HorizontalSlider(new Rect(140f, num5 + 7f, width - 220f, 30f), ApplicationDataManager.ApplicationOptions.CameraFov, ApplicationOptions.CameraFovSliderMin, ApplicationOptions.CameraFovSliderMax, BlueStonez.horizontalSlider, BlueStonez.horizontalSliderThumb);
		num7 = Mathf.Round(num7);
		if (Mathf.Abs(num7 - ApplicationDataManager.ApplicationOptions.CameraFov) >= 0.5f)
		{
			ApplicationDataManager.ApplicationOptions.CameraFov = num7;
			if (Camera.main != null)
			{
				Camera.main.fieldOfView = num7;
			}
			graphicsChanged = true;
		}
		GUI.Label(new Rect(width - 70f, num5, 70f, 30f), num7.ToString("f0"), BlueStonez.label_interparkbold_11pt_left);
		num5 += 30;
		if (!false || showResolutions)
		{
