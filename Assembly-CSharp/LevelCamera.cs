...
public class LevelCamera : MonoBehaviour
{
	...
	private class RagdollState : CameraState
	{
		...
		public RagdollState(LevelCamera camera, Vector3 offset)
			: base(camera, offset)
		{
			Avatar avatar = (GameState.Current != null) ? GameState.Current.Avatar : null;
			AvatarDecoratorConfig ragdoll = (avatar != null) ? avatar.Ragdoll : null;
			Transform hips = (ragdoll != null) ? ragdoll.GetBone(BoneIndex.Hips) : null;
			targetPosition = ((hips != null) ? hips.position : camera.Transform.position) + new Vector3(0f, 1.5f, 0f);
		}
	...
	private void SetCamera(Camera camera, Vector3 position, Quaternion rotation)
	{
		if (camera != MainCamera && camera != null)
		{
			...
			if (ApplicationDataManager.ApplicationOptions != null && ApplicationDataManager.ApplicationOptions.CameraFov >= ApplicationOptions.CameraFovSliderMin)
			{
				MainCamera.fieldOfView = ApplicationDataManager.ApplicationOptions.CameraFov;
				_cameraConfiguration.Fov = MainCamera.fieldOfView;
			}
	...
	private void SetCameraMode(CameraMode mode, Transform target)
	{
		...
		case CameraMode.Ragdoll:
			MainCamera.cullingMask = LayerUtil.AddToLayerMask(MainCamera.cullingMask, UberstrikeLayer.LocalPlayer);
			// Null guard: if the Avatar or its Ragdoll wasn't spawned (e.g.,
			// SpawnRagdoll early-returned due to destroyed Decorator, see
			// Avatar.cs:82), GetBone would NullRef. Fall back to the
			// player transform so the kill camera still functions.
			if (GameState.Current.Avatar != null && GameState.Current.Avatar.Ragdoll != null)
			{
				TargetTransform = GameState.Current.Avatar.Ragdoll.GetBone(BoneIndex.Hips);
			}
			else if (GameState.Current.Player != null)
			{
				TargetTransform = GameState.Current.Player.transform;
			}
			_currentState = new RagdollState(this, new Vector3(0f, 1f, 0f));
			if (GameState.Current.Player != null && (bool)GameState.Current.Player.WeaponCamera)
			{
				GameState.Current.Player.WeaponCamera.IsEnabled = false;
			}
			break;
