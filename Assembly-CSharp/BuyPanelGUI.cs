...
public class BuyPanelGUI : PanelGuiBase
{
	...
	{
		// Offline editor bypass: simulate a successful purchase locally by
		// rebuilding the inventory list with the new item appended, then
		// calling UpdateInventoryItems + EquipItem. Skips the server SOAP
		// call (which would reject our OFFLINE_TOKEN auth) and the inventory
		// refresh that would otherwise erase our local grant.
		if (PlayerDataManager.AuthToken == "OFFLINE_TOKEN")
		{
			try
			{
				List<ItemInventoryView> invViews = new List<ItemInventoryView>();
				foreach (InventoryItem invItem in Singleton<InventoryManager>.Instance.GetAllItems(false))
				{
					if (invItem != null && invItem.Item != null)
					{
						invViews.Add(new ItemInventoryView { ItemId = invItem.Item.View.ID, AmountRemaining = invItem.AmountRemaining });
					}
				}
				invViews.Add(new ItemInventoryView { ItemId = item.View.ID, AmountRemaining = 0 });
				Singleton<InventoryManager>.Instance.UpdateInventoryItems(invViews);
				if (autoEquip)
				{
					Singleton<InventoryManager>.Instance.EquipItem(item);
				}
				Debug.Log("[Offline] Free purchase: " + item.View.Name + " (id=" + item.View.ID + ") autoEquip=" + autoEquip);
			}
			catch (Exception ex)
			{
				Debug.LogWarning("[Offline] BuyItem bypass failed: " + ex.Message);
			}
			_isBuyingItem = false;
