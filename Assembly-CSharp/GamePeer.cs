...
public class GamePeer : BaseGamePeer
{
	...
	public void CreateGame(GameRoomData data, string password)
	{
		if (base.IsConnected)
		{
			base.Operations.SendCreateRoom(data, password, "4.8.6", PlayerDataManager.AuthToken, PlayerDataManager.MagicHash);
			return;
		}
	...
	public void JoinGame(string server, int roomId, string password = "")
	{
		...
		if (base.IsConnected)
		{
			base.Operations.SendEnterRoom(roomId, password, "4.8.6", PlayerDataManager.AuthToken, PlayerDataManager.MagicHash);
			return;
		}
		onConnectAction = delegate
		{
